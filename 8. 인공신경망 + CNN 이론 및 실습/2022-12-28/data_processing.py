...
print(len(train_data)) # train data 갯수 : 567개
print(len(test_data)) # test data 갯수 : 310개

train_image_resize = True
if train_image_resize == True :
    for i in train_data :
        f_name = i.split("/")
        f_name = f_name[3] # ['.', 'dataset', 'train_image', '3', 'three_00141.jpg'] --> 폴더명으로 쓸 순수 숫자를 갖다쓸 것
        # print(type(f_name))
        if f_name == "0" :
            img = Image.open(i)
...
if test_image_resize == True :
    for i in test_data :
        f_name = i.split("/")
        f_name = f_name[3] # ['.', 'dataset', 'test_image', '3', 'three_00141.jpg'] --> 폴더명으로 쓸 순수 숫자를 갖다쓸 것
        # print(type(f_name))
        if f_name == "0" :
            img = Image.open(i)
            img_new = expand2square(img, (0,0,0)).resize((400,400))
