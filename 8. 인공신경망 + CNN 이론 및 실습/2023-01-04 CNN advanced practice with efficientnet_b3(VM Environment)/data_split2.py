...
def create_train_val_split_folder(path) :
    ...
    os.makedirs("./dataset01/train/" , exist_ok=True)
    os.makedirs("./dataset01/val/", exist_ok=True)

    for category in sorted(all_categories) :
        os.makedirs(f"./dataset01/train/{category}", exist_ok=True)
        ...
        for image in random.sample(all_image, int(0.9 * len(all_image))) :
            shutil.move(f"./data/{category}/{image}",
                        f"./dataset01/train/{category}/")
    for category in sorted(all_categories) :
        os.makedirs(f"./dataset01/val/{category}", exist_ok=True)
        all_image = os.listdir(f"./data/{category}/")
        for image in all_image :
            shutil.move(f"./data/{category}/{image}",
                        f"./dataset01/val/{category}/")
