...
class Ui_MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 버튼을 눌렀을 때만 그릴 수 있도록 bool 변수를 하나 선언한다
        self.is_paint = False

        # 사각형 박스의 좌표를 저장할 리스트 선언
        self.bbox_list = []
    ...
    def img_open(self):
        ...
        if file_dialog.exec() == QDialog.Accepted:
            ...
            # 메뉴바 크기도 알아야 됨. 실제 0,0은 메뉴바에 가려짐
    ...
    def prev_bbox(self) :
        ...
        # 새로 그리기 위해서 pixmap을 재정의한다.
        self.pixmap = QPixmap(self.img_url.toLocalFile())
        self.pixmap = self.pixmap.scaled(self.img_label.size(), aspectMode=Qt.KeepAspectRatio)

        self.bbox_list = self.bbox_list[:-1] # 마지막 꺼 제외하고 다시 그림
        """지워진다기 보다 지워진 상자를 제외하고 새로 그린다고 이해하면 좀 낫다"""
    ...
    def paintEvent(self, event):
        if self.pixmap :
            with QPainter(self) as painter: # 얘가 전체 창(main widget)을 대상으로 함수를 실행하므로
                painter.drawPixmap(self.set_p_x, self.set_p_y, self.pixmap)

                if self.is_paint :
                    painter.setBrush(self.br)
                    painter.drawRect(QRect(self.begin, self.end)) # 얘가 좌표가 아래와 다르게 self.begin, end로 실행됨
    ...
    def mouseMoveEvent(self, event) :
        ...
        self.update() # 미리보기를 그려줬으면 하는데 화면 갱신을 위해 작성
    ...
    def mouseReleaseEvent(self, event) :
        ...
        if self.pixmap and self.is_paint :
            ...
            # QRect가 x, y, w, h
            self.painter.drawRect(QRect(begin[0], begin[1], w, h)) # pixmap 상에서의 0,0과 전체 창에서의 0,0이 다름
            self.painter.end()
    ...
    def closeEvent(self, close_event) :
        self.deleteLater()
        close_event.accept()
        # close_event.ignore() # 진짜 끌거냐? 물어보는 부분


if __name__ == "__main__" :
    app = QApplication(sys.argv)
    # app.setStyle('Fusion') # 테마 변경하고 싶으면 주석 풀고 써라
